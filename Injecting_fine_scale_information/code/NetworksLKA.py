...
#         print(scale_factor)
        ...
        self.conv_bf_end = nn.Conv3d(2 ** (wg_sf - 2), 2, 3, stride=1, padding=1, padding_mode='replicate')
...
class LKA(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = nn.Conv3d(dim, dim, (5, 5, 5), padding=(2, 2, 2), groups=dim)
        self.conv_spatial = nn.Conv3d(dim, dim, (7, 7, 7), stride=1, padding=(9, 9, 9), groups=dim, dilation=(3, 3, 3))
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(batch_size, channels, 1, 1, 1)
